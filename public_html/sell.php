...
foreach ($listAllOpenOrders as $openOrders) {
    ...
    $currencyData = $repository->getCurrency($pairTo);
    ...
    foreach ($openOrders->getOrders() as $order) {
        if ($currencyData['max_price'] < $sumNow) {
            $repository->updateCurrency($pairTo, $sumNow);
        }
        $diffMaxSumProc = 100 - (($currencyData['max_price'] * 100) / $sumNow);
